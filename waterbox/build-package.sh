#!/bin/sh
# Builds the ares waterbox core package and installs it into a miniHawk
# checkout as build/Cores/ares.chimeraCore.
#
...
ninja -C "$root/build/meson-guest"
sh "$mb/source/guest/check-wbx.sh" "$root/build/meson-guest/waterbox/core.wbx"

staging="$out/package-staging"
rm -rf "$staging"
mkdir -p "$staging"
cp "$root/build/meson-guest/waterbox/core.wbx" "$staging/core.wbx"
...
mkdir -p "$cores_dir"
zip_path="$cores_dir/ares.chimeraCore"
rm -f "$zip_path"
...
for cache in "$minihawk_root"/build/UnpackedCores/ares-*; do
